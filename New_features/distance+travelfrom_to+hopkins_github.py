# -*- coding: utf-8 -*-
"""Distance+Travelfrom_to+Hopkins_Github.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1c7sW37k4oLiakuOWpFLmdNxi35AC5nMW
"""
...
Suggestion for Mykola to possibly populate the dashboard
"""

#distance to the dashboard
df1 = pd.DataFrame(columns=['sourcename', 'targetname', 'distance']) #test ds

row = {"sourcename": "Berlin", "targetname": "Moscow", "distance": 2000}
df1 = df1.append(row, ignore_index=True)
df1

row = {"sourcename": "Munchen", "targetname": "Moscow", "distance": 2500}
df1 = df1.append(row, ignore_index=True)

cities_distance = {} #empty dict
for row in range(0, len(df)): #iterate
  k = df["sourcename"][row] + "_" + df["targetname"][row] #concatenate
  v = df["distance"][row]
...
#function for getting all the distances
...
# distance = get_distance("Berlin", "Moscow")
distance = get_distance(sourceName, targetName) #in general
st = ":arrow: To reach the final destination you need to cover: " + str(distance) + " km"
print(st)

"""Might be helpful to see the distance on dashboard using the car for traveling=> traveler could calculate the petrol expenses knowing the km"""