...
def calculate_caloriesBurnt(df, weight = 65):
    time_inSec = df['totalTravelTimeInSec'].values[0] #.values[0] --> used to only get the value not the index information
    ...
    if "walkingDistance" in preferences_list:
        walk_dist = df['walkingDistance'].values[0]
        #convert walking distance to walking time (4.83km/h = avg. walking speed)
        walk_time = (walk_dist/4.83) * 3600
        #subtract the walking time from the passive sitting time
        travel_time_passive = time_inSec - walk_time
        #add scores of passive traveling and walking
        met_score = (travel_time_passive/3600 * 1.3) + (walk_time/3600 * 3.5)
    else:
        #only passive travelling (sitting activity)
        met_score = (time_inSec/3600) * 1.3

    return get_calories(met_score, weight)

###only for testing purposes###
path_source = "/home/lisnux/Desktop/UniWien/WS2122/DS_Project/data/reduced_datasets/3dim/combined_csvs/"
df_3dim = pd.read_csv(path_source+"combined_csv_3dim.csv", encoding = 'utf-8')
