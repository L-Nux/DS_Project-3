...
from sklearn.preprocessing import StandardScaler
scaled_new = StandardScaler().fit_transform(scaled[['totaltraveltimeinhours', 'totalprice','totalnumberofchanges','totalwalkingdistance','totalwaitingtime']])
scaled[['totaltraveltimeinhours', 'totalprice','totalnumberofchanges','totalwalkingdistance','totalwaitingtime']] = scaled_new
...
catColumnsPos = [df.columns.get_loc(col) for col in list(scaled.select_dtypes('object').columns)]
print('Categorical columns           : {}'.format(list(scaled.select_dtypes('object').columns)))
print('Categorical columns position  : {}'.format(catColumnsPos))

# Convert dataframe to matrix- we need it for kprototypes input
dfmatrix = scaled.to_numpy()
dfmatrix

#install the library
!pip install kmodes

#to check how mny clusters?
...
#obtained the result that the best number for clusters is 5 
...
#get the fisrt 15 values for cluster 0, to understand a rough trend
#how to visualize to explain?
df5[df5['cluster']==3].head (20)

# define and map colors to visulize the clusters
#colors = ['#DF2020', '#81DF20', '#2095DF','#d02d30','#fce903']
...
#investigate the 4th group (3d in clustering), which has really interesting parameters
#subset the data
fourth = df5[df5["cluster"] == 3]
fourth
...
sns.boxplot(data=fourth,x="finalsolutionusedlabels", y='totalprice',palette='Set3')#take care about variables of the data
plt.title('Price distribution per 4th group')
