# -*- coding: utf-8 -*-
"""multilabel_classification_final (1).ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1uTOAhDBOpajdsDSe2Kcj9jxc6cQuDh9-
"""
...
norm = MinMaxScaler()
train_X_norm = norm.fit_transform(X_train) #use fit_trasform for training data
train_X_norm
...
For this reason, tried using "compute_sample_weight" from "class_weight" instead with balancing approach for imbalanced classes/ labels, since 1 sample is belonging to >than 1 class, could handle multi-label output problems
...
norm = MinMaxScaler()
train_X1_norm = norm.fit_transform(X1_train) #use fit_trasform for training data
train_X1_norm
...
model1.compile(loss=binary_crossentropy,
              optimizer=Adam(learning_rate=0.01), #tune learning rate=0.000001=the result got worse
              #metrics=['categorical_accuracy']
              #metrics = [tf.keras.metrics.BinaryAccuracy(),tf.keras.metrics.CategoricalAccuracy(),tf.keras.metrics.AUC()]) #for binary_accuracy =0,92
              metrics = [tf.keras.metrics.CategoricalAccuracy(),tf.keras.metrics.AUC()])
              #metrics=[tf.keras.metrics.Precision()])
...
Precision- we got an extremely high precision of 1-0,82 for the 4 classes (blablacar,car,db_fv,flixbus) (correctly predicted positive to the total positive)=>low false positive rate there, 
...
def extract_rows_by_type(t, df): #extract rows that contain the word per mode in separate df for class (label)upsampling the minorities
  df_by_type = df[0:0]
...
norm = MinMaxScaler()
train_X2_norm = norm.fit_transform(X2_train) #use fit_trasform for training data
train_X2_norm
...
model3.compile(loss=binary_crossentropy,
              ...
              #metrics=['categorical_accuracy']
              #metrics = [tf.keras.metrics.BinaryAccuracy(),tf.keras.metrics.CategoricalAccuracy(),tf.keras.metrics.AUC()]) #for binary_accuracy =0,96
              metrics = [tf.keras.metrics.CategoricalAccuracy(),tf.keras.metrics.AUC()])
              #metrics=[tf.keras.metrics.Precision()])
...
Possible improvement for the future work-make the testing set relatively bigger
"""
